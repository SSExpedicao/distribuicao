...
def extrair_texto_arquivo(arquivo_upload):
    try:
        ...
        if nome.endswith('.txt'):
            ...
            st.error(f"Formato nao suportado: {nome}. Use PDF, DOCX ou TXT.")
            return None
...
def verificar_criar_tabelas():
    ...
    VALUES ('secretario','123','Secretario das Sessoes','Secretario','GAB','Raiz',TRUE),
           ...
           ('andre.sexp','123','Andre Assessor','Assessor','SEXP','Operacional',TRUE)
    ...
    CREATE TABLE IF NOT EXISTS public.equipe (
        ...
        expedicao TEXT DEFAULT 'Nao', revisao TEXT DEFAULT 'Nao',
    ...
    CREATE TABLE IF NOT EXISTS public.pauta_seat (
        ...
        urgente INT DEFAULT 0, status TEXT DEFAULT 'Em Edicao',
    ...
    CREATE TABLE IF NOT EXISTS public.pauta_sexp (
        ...
        urgente INT DEFAULT 0, status TEXT DEFAULT 'Aguardando Homologacao Chefia',
    ...
    CREATE TABLE IF NOT EXISTS public.pauta_sercon (
        id SERIAL PRIMARY KEY, processo TEXT, relator TEXT,
        motivo_gatilho TEXT, status TEXT DEFAULT 'Pendente Analise Contabil',
        analista_responsavel TEXT, data_entrada TEXT, data_conclusao TEXT,
        observacao TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
    );
    CREATE TABLE IF NOT EXISTS public.pauta_semand (
        id SERIAL PRIMARY KEY, processo TEXT, relator TEXT,
        destinatario TEXT, status TEXT DEFAULT 'Aguardando Expedicao de Mandado',
    ...
    try:
        ...
        st.warning("Nao foi possivel criar tabelas automaticamente. Execute o SQL manual no Supabase SQL Editor.")
