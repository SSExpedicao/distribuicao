...
def renomear_sessao(nome_antigo, novo_nome, tipo_sessao_alvo):
    try:
        ...
        return True, f"✅ Número updated para: {novo_nome}"
    except Exception as e:
...
def obter_revisor(expedidor, nome_sessao, revisores_ativos):
    ...
    try:
        ...
        for cand in candidatos:
            ...
            text_sessao_count = len(df_sessao[df_sessao['revisao'] == cand]) if 'revisao' in df_sessao.columns else 0
            vezes_parceiro = 0
            carga_total = len(df_total[df_total['revisao'] == cand]) if 'revisao' in df_total.columns else 0

            score = (parcerias_sessao, is_reciprocal, text_sessao_count, vezes_parceiro, carga_total)
...
def adicionar_aviso(usuario, numero_processo, message):
    try:
        ...
        agora = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        conn.table("avisos").insert({"usuario": usuario, "numero_processo": numero_processo, "mensagem": message, "data_criacao": agora, "ativo": 1}).execute()
