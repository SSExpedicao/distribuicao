    ...
    try:
        ...
        for p in res:
            ...
            if tipo_sessao_original == "Sessão Administrativa":
                ...
                if not exp_filtrados: exp_filtrados = [n for n in cargos.keys() if n not in ausentes]
            ...
            if revisores_deste_expedidor:
                ...
                revisores_ocupados = list(set([row.get('revisao') for row in urgentes_ativos if row.get('expedicao') != responsavel_expedicao and row.get('revisao')]))
                quem_manda_pra_mim = list(set([row.get('expedicao') for row in urgentes_ativos if row.get('revisao') == responsavel_expedicao and row.get('expedicao')]))
                ...
                if not candidatos_rev:
                    ...
                    candidatos_rev = [n for n in pool_socorro if n not in revisores_ocupados and n not in quem_manda_pra_mim]
                    
                    if not candidatos_rev:
                        candidatos_rev = pool_socorro
                        if not candidatos_rev:
...
def salvar_novo_processo(numero_processo, relator, tipo_sessao, nome_sessao, expedidores_selecionados, revisores_selecionados):
    ...
    # --- LÓGICA DO REVISOR E BUSCA DE SOCORRO NO BANCO ---
